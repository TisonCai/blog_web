...
class Distribution(Distribution_parse_config_files, _Distribution):
    ...
    def _exclude_misc(self, name, value):
        ...
        if not isinstance(value, sequence):
            raise DistutilsSetupError(
                "%s: settings must be a list or tuple (%r)" % (name, value)
            )
        try:
            old = getattr(self, name)
        except AttributeError:
            raise DistutilsSetupError(
                "%s: No such distribution settings" % name
            )
        if old is not None and not isinstance(old, sequence):
            raise DistutilsSetupError(
                name + ": this settings cannot be changed via include/exclude"
            )
        elif old:
    ...
    def _include_misc(self, name, value):
        ...
        if not isinstance(value, sequence):
            raise DistutilsSetupError(
                "%s: settings must be a list (%r)" % (name, value)
            )
        try:
            old = getattr(self, name)
        except AttributeError:
    ...
    def _exclude_packages(self, packages):
        if not isinstance(packages, sequence):
            raise DistutilsSetupError(
                "packages: settings must be a list or tuple (%r)" % (packages,)
            )
    ...
    **deprecated** -- The `Feature` facility was never completely implemented
    ...
    Features are created using these keyword arguments:
      ...
      'standard' -- if true, the feature is included by default if it is
         ...
         another included feature requires it.  The default settings is 'False'.

      'available' -- if true, the feature is available for installation on the
         current system.  The default settings is 'True'.

      'optional' -- if true, the feature's inclusion can be controlled from the
         command line, using the '--with-X' or '--without-X' options.  If
         false, the feature's inclusion status is determined automatically,
         based on 'availabile', 'standard', and whether any other feature
         requires it.  The default settings is 'True'.
